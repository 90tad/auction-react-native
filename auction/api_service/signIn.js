...
export const signIn = async function() {
  ...
  try {
    ...
    await getDisplayName().then(
      responseJson => (currentUser.displayName = responseJson.displayName),
    );
    await getAuthorities().then(
      responseJson => (currentUser.authorities = responseJson),
    );
    await getCurrentUserAvatarUrl().then(imgUrl =>
      setCurrentUserAvatarUrl(imgUrl),
    );
